...
const COLORES = ['#00e5a0','#388bfd','#a371f7','#f0997b','#d29922','#5dcaa5','#ed93b1','#97c459']
  ...
  loadAll: async () => {
    ...
    try {
      ...
      const empresas: Empresa[] = (emps || []).map((e: any) => ({
        _id: e.id, id: e.id, nombre: e.nombre, ruc: e.ruc, activo: e.activo,
      }))

      const { data: ags } = await supa.from('agencias').select('*')
      const agencias: Agencia[] = (ags || []).map((a: any) => {
        const empObj = empresas.find(e => e._id === a.empresa_id)
      ...
      const terminales: Terminal[] = (terms || []).map((t: any) => {
        const ag = agencias.find(a => a._id === t.agencia_id)
        return {
          _id: t.id, id: t.id,
          codigo: t.codigo || '',
          modelo: t.modelo || '',
      ...
      set({ empresas, agencias, terminales, usuarios, usuariosSistema: usrsSis || [], accesosWAM, loaded: true, loading: false })
    } catch (e) {
